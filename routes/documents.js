 ...
 * the server restarts mid-job; POST /:id/process is how a user recovers from
 * that. Moving to a standalone worker later means changing only the trigger,
 * not the pool.
 */
...
// Memory storage: the buffer goes to pdfjs and then to Supabase storage, so it
// never needs a path on this disk. MAX_PDF_BYTES is what keeps that honest --
// without a cap, one upload could exhaust the process heap.
const upload = multer({
...
// %PDF at byte 0. mimetype is client-supplied and trivially spoofed, so the
// bytes get the final say before anything is parsed or stored.
...
/**
 * Turn cleaned text into chunk rows, marking the ones already in this user's
 * cache as done up front.
 *
 ...
 */
async function buildChunkRows(supabase, userId, documentId, text) {
  const chunks = chunkText(text, { maxChars: MAX_CHUNK_CHARS });
  if (chunks.length === 0)
    return { rows: [], billableChars: 0, cachedCount: 0 };
  ...
  return { rows, billableChars, cachedCount };
}

/**
 * POST /api/documents
 *
 * multipart/form-data with `file` (a PDF) and optional `title`, or
 * application/json with { title, text } for pasted text.
 *
 * Extracts, cleans, chunks, and queues a job. Returns immediately -- synthesis
 * happens in the background, because a 40-page chapter takes minutes and no
 * HTTP client should be asked to hold a connection open that long.
 */
  ...
  try {
    ...
    if (req.file) {
      ...
      try {
        ...
        // A corrupt or encrypted PDF is the user's problem to fix, not a server
        // fault -- say so with a 400 rather than a 500.
        return res.status(400).json({
          error: `Could not read that PDF: ${err.message}`,
        });
      }

      text = cleanPages(pages);
      ...
      return res
        .status(400)
        .json({ error: "Send a PDF file or a text field." });
    }
    ...
    let built;
    try {
      built = await buildChunkRows(req.supabase, userId, doc.id, text);
    } catch (err) {
    ...
    if (built.rows.length === 0) {
      await cleanup("no chunks produced");
      return res.status(400).json({ error: "No readable text found." });
    }

    // Only unsynthesised characters count -- cached ones cost nothing.
    const quota = await checkQuota(req.supabase, userId, built.billableChars);
    if (!quota.ok) {
      await cleanup("quota exceeded");
      return res.status(429).json({
        error:
          `This document needs ${built.billableChars} characters but only ` +
          `${quota.remaining} remain of your ${quota.quota}/day. ` +
          `Try a shorter document or wait for the window to roll over.`,
        needed: built.billableChars,
    ...
    if (pdfBuffer) {
      const sourcePath = `${userId}/sources/${doc.id}.pdf`;
      const { error: upErr } = await req.supabase.storage
        .from("library")
        .upload(sourcePath, pdfBuffer, {
    ...
    const pending = built.rows.length - built.cachedCount;
    ...
    res.status(201).json({
      ...
      chunks: {
        ...
        cached: built.cachedCount,
        pending,
      },
      billableChars: built.billableChars,
      quota: {
        used: quota.used,
        remaining: quota.remaining,
        limit: quota.quota,
...
router.get("/:id", requireAuth, async (req, res) => {
  ...
  const [{ data: progress }, { data: jobs }, { data: asset }] =
    await Promise.all([
...
module.exports = { router, buildChunkRows, looksLikePdf };
