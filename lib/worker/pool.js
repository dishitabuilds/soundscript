 ...
 *
 * @param {{ provider?: string, voiceId?: string, modelId?: string }} [ttsSettings]
 *   the document's voice; defaults apply when absent
 */
async function processChunk(supabase, userId, chunk, log, ttsSettings = {}) {
  ...
  const audio = await withRetry(() => synthesize(chunk.text, ttsSettings), {
...
async function workerLoop(
  supabase,
  { userId, documentId, jobId, state, log, ttsSettings },
) {
  for (;;) {
...
async function processDocument(supabase, options) {
  ...
  // The document row owns the voice. Read once here, not per chunk -- every
  // chunk of a document speaks with the same narrator by construction, since
  // the chunk hashes were computed from these settings at upload time.
  const { data: doc } = await supabase
    .from("documents")
    .select("provider, voice_id, model_id")
    .eq("id", documentId)
    .maybeSingle();

  const ttsSettings = {
    provider: doc?.provider || undefined,
    voiceId: doc?.voice_id || undefined,
